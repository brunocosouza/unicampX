...
class PascalVOCSingleObject(InMemoryDataset):
    ...
    def __init__(self, root, category, train=True, transform=None,
        ...
        super(PascalVOCSingleObject, self).__init__(root, transform,
    ...
    def process(self):
        ...
        #Single object retrieved train
        train_single = []
        train_split.sort()
        for i, name in enumerate(train_split):
            if int(name.split('_')[-1].split('.')[0]) - 1 < 1:
                train_single.append(name)
            elif (int(name.split('_')[-1].split('.')[0]) - 1) == 1 and len(train_single)>0:
                train_single.pop(len(train_single) - 1)
            else : pass

        #Single object retrieved train
        test_single = []
        test_split.sort()
        for i, name in enumerate(test_split):
            if int(name.split('_')[-1].split('.')[0]) - 1 < 1:
                test_single.append(name)
            elif int(name.split('_')[-1].split('.')[0]) - 1 == 1 and len(test_single)>0:
                test_single.pop(len(test_single) - 1)
            else : continue
        ...
        train_set, test_set = [], []
        for i, name in enumerate(chain(train_single, test_single)):
            filename = '_'.join(name.split('/')[1].split('_')[:-1])
            idx = int(name.split('_')[-1].split('.')[0]) - 1
            if self.category != 'person':
                assert idx == 0 #Assert only 1 object per image
            ...
            img_path = osp.join(image_path, '{}.jpg'.format(filename))
            img = read_image(impath = img_path, img_size = [256,256])
